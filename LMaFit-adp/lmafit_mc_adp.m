...
for iter = 1:maxit
    ...
    objv(iter) = (relres*datanrm)^2;    
    if(iter <= 4)
        delta = inf;
    else
        delta = objv(iter - 4:iter - 1) - objv(iter - 3:iter);
        delta = abs(delta)/objv(iter);
    ...
    if iprint == 2;
        fprintf('it: %5i rk: %4d, rel. %3.1e r. %4.4f chg: %3.1e alf: %3.1e inc: %3.1e\n',...
            iter, k, relres,ratio,delta,alf0x,increment);
    end
    
    % testing performance
    if(isfield(opts, 'test'))
        tempS = eye(size(X, 2), size(Y, 1));
        if(opts.test.m ~= m)
            RMSE(iter) = MatCompRMSE(Y', X, tempS, opts.test.row, opts.test.col, opts.test.data);
        else
            RMSE(iter) = MatCompRMSE(X, Y', tempS, opts.test.row, opts.test.col, opts.test.data);
        end
        fprintf('RMSE %.2d \n', RMSE(iter));
    end
    Time(iter) = toc(timeFlag);
