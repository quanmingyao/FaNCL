...
if isempty(Q_k) % New Lanczos tridiagonalization.
   ...
   if isfinite(delta)
      ...
      if (exist('mexreorth') == 3)
         [r,beta(j+1),rr] = mexreorth(Q_k,r,beta(j+1),int,gamma,cgs);
      else
...
for j=j0:kmax,  
   ...
   elseif isstruct(A)
      ...
      if ~isempty(Apar); u = feval(A,q,Apar);
      else; u = feval(A,q);
      end
   end
   r = u - beta(j)*q_old;
  ...
  if deflate    
     if (exist('mexreorth') == 3)
        [r,beta(j+1),rr] = mexreorth(options.Y,r,beta(j+1),1:size(options.Y,2),...
			   gamma,cgs);
     else
  ...
  if (j<kmax) & (beta(j+1) < n*anorm*eps)
     ...
     for attempt=1:3    
	   ...
	   if ~isempty(Apar); r = feval(A,r,Apar);
	   else;  r = feval(A,r);
           end
        end      
        ...
        int = 1:j;
        if (exist('mexreorth') == 3)
           [r,nrmnew,rr] = mexreorth(Q_k,r,nrm,int,gamma,cgs);
        else
