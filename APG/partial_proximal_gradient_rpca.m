function [X_k_A,X_k_E,numIter] = partial_proximal_gradient_rpca(D, lambda, mu, ...
    maxIter, tol, lineSearchFlag, eta)
...
while ~converged
    ...
    if ~lineSearchFlag
        ...
        diagS = diag(S);
        svp = length(find(diagS > mu/tau_k));
        if svp < sv
            sv = min(svp + 1, n);
        else
        ...
        X_kp1_A = U(:, 1:svp) * diag(diagS(1:svp) - mu/tau_k) * V(:, 1:svp)';
    
        X_kp1_E = sign(X_km1_E) .* pos( abs(X_km1_E) - lambda*mu/tau_k );
        
        rankA  = sum(diagS>mu/tau_k);
        ...
        while ~convergedLineSearch
            ...
            [U,S,V] = svd(G_A,'econ') ;
            diagS = diag(S) ;
            SG_A = U * diag(pos(diagS-mu/tau_hat)) * V';
            SG_E = sign(G_E) .* pos( abs(G_E) - lambda*mu/tau_hat );
